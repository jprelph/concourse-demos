...
if [ "${ENABLE_WEB_SOCKETS}" == "TRUE" ]; then
    ...
    WEB_SOCKETS="include ${NGINX_CONF_DIR}/nginx_web_sockets_proxy.conf;"
else
    unset WEB_SOCKETS
fi
...
if [ "${REQS_PER_MIN_PER_IP}" != "" ]; then
    ...
    echo "limit_req_zone \$${REMOTE_IP_VAR} zone=reqsbuffer${LOCATION_ID}:10m rate=${REQS_PER_MIN_PER_IP}r/m;" \
        >${NGINX_CONF_DIR}/nginx_rate_limits_${LOCATION_ID}.conf
...
location ${LOCATION} {
    ...
    ${WEB_SOCKETS}
    $(cat /etc/nginx/conf/location_template.conf)
    ${SSL_CERTIFICATE}
    ${SSL_VERIFY}
